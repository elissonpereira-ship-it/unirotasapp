...
function m_init() {
    ...
    else if (m_state.location && m_state.location.id) {
...
async function m_openReturnModal() {
    console.log("🔄 Preparando fluxo de retorno...");
    
    // Se o usuário está em modo individual (Solo)
    if (m_state.role === 'individual' || !m_state.currentSession) {
        const profile = window._userProfile;
        
        if (!profile) {
            return m_showToast("Perfil não carregado para obter endereço.", "error");
        }

        // Montagem do endereço residencial (conforme campos da tabela usuarios)
        const rua = profile.address_rua || "";
        const num = profile.address_num || "";
        const bairro = profile.address_bairro || "";
        const cidade = profile.address_cidade || "";

        if (!rua || !cidade) {
            return alert("Endereço residencial não encontrado no seu perfil.\nPor favor, atualize seus dados no menu principal.");
        }

        const fullAddress = `${rua}, ${num}, ${bairro}, ${cidade}`.replace(/, ,/g, ',').trim();
        
        // Origem: Localização Atual (GPS) ou nome do local da reunião como fallback
        const origin = (window.lastLat && window.lastLon) ? `${window.lastLat},${window.lastLon}` : (m_state.location?.name || "Minha Localização");
        
        const mapsUrl = `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(origin)}&destination=${encodeURIComponent(fullAddress)}&travelmode=driving`;
        
        console.log("🚗 Abrindo navegação de retorno (Solo):", mapsUrl);
        window.open(mapsUrl, '_blank');

        // Para o modo solo, finalizamos a jornada localmente após abrir o mapa
        m_showToast("Retorno iniciado! Dirija com cuidado.", "success");
        m_cleanupSession();
    } 
    else {
        // Se for motorista com caronas, abre o modal de opções
        const modal = document.getElementById('m-modal-return-setup');
        if (modal) {
            modal.classList.add('show');
            modal.style.display = 'flex';
        } else {
            // Fallback caso o modal não exista: inicia com os mesmos caronas
            m_setupReturn('same');
        }
    }
}

async function m_setupReturn(type) {
    console.log("🛠️ Configurando retorno do tipo:", type);
    closeModal('m-modal-return-setup');

    if (type === 'new') {
        // Abre tela de busca para selecionar novos caronas (reutiliza m-view-search)
        m_state.paxSelected = [];
        m_showView('m-view-search');
        m_showToast("Selecione os novos caroneiros.", "info");
    } else {
        // Inicia retorno com os mesmos passageiros da ida
