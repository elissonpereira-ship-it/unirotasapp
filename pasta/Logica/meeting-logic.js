...
function m_renderOutboundList() {
    ...
    container.innerHTML = paxes.map(p => {
        ...
        return `
            <div class="pax-item-v2 ${isChecked ? 'checked' : ''}" style="position:relative;">
                ...
                <div data-menu-id="${p.uid}" class="pax-actions-menu">
                    ...
                    <div class="pax-menu-item danger" onclick="m_removePaxFromSession('${p.uid}')" style="border-top:1px solid rgba(255,255,255,0.05);">
...
function m_subscribeToSession() {
    ...
    m_sessionChannel = window.supabase
        ...
        .on('broadcast', { event: 'pax_canceled' }, async payload => {
            ...
            console.log("👤 [Realtime] Passageiro cancelou:", paxUid);
            if (m_state.role === 'driver') await m_refreshSessionData();
            else m_renderOutboundList();
        })
        .subscribe();
}
