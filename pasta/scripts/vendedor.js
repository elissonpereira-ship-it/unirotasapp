...
async function handleAuth() {
    ...
    try {
        if (isSignupMode) {
            ...
            const cpf = document.getElementById('signup-cpf-input')?.value.trim();
            const emailReg = document.getElementById('user-email-input')?.value.trim();
            ...
            const fullAddress = `${rua}, ${num} - ${bairro}, ${cidade} - ${cep}`;

            if (!name || !emailReg) throw new Error("Informe nome e e-mail");

            const { data, error } = await getSupa().auth.signUp({ 
                email: emailReg, 
                password,
                options: { data: { full_name: name, city: cidade } }
            });
...
function startTracking() {
    ...
    watchId = navigator.geolocation.watchPosition(async pos => {
        ...
        if (typeof window.recordGpsPoint === 'function') {
            window.recordGpsPoint(lat, lon);
        }
