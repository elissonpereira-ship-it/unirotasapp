        ...
        .on(
            ...
            payload => {
                if (payload.eventType === 'INSERT') {
                    ...
                    console.log("🗑️ Remoção detectada via Realtime:", oldId);

                    // Procuramos em todas as threads e removemos a mensagem com este ID
                    let found = false;
                    Object.keys(allChatMessages).forEach(vUid => {
                        if (allChatMessages[vUid][oldId]) {
                            delete allChatMessages[vUid][oldId];
                            found = true;

                            // Se esta thread for a ativa, re-renderiza o chat
                            if (activeChatVendorId && activeChatVendorId.toString().toLowerCase() === vUid) {
                                renderActiveChat(allChatMessages[vUid]);
                            }
                        }
...
async function openMeetingAttendanceModal() {
    ...
    // Popula filtro de datas a partir do Supabase
    const dateFilter = document.getElementById('attendance-date-filter');
    const locFilter = document.getElementById('attendance-location-filter');

    if (dateFilter) {
        dateFilter.innerHTML = '<option value="">Carregando...</option>';
        try {
            ...
            // Extrai datas únicas do campo 'date' direto
            const uniqueDates = [...new Set((sessions || []).map(s => s.date))].filter(d => !!d);

            if (uniqueDates.length === 0) {
                dateFilter.innerHTML = '<option value="">Nenhuma reunião encontrada</option>';
            } else {
                dateFilter.innerHTML = uniqueDates.map(d => {
            ...
            dateFilter.innerHTML = '<option value="">Erro</option>';
        }
    }

    // Popula filtro de locais a partir do Supabase
    if (locFilter) {
        locFilter.innerHTML = '<option value="all">Todos os Locais</option>';
        try {
            ...
            (locs || []).forEach(l => {
                ...
                opt.value = l.name; // Usa o NOME como valor (campo salvo em meeting_sessions)
...
async function loadAttendanceList() {
    ...
    try {
        // Busca sessões do Supabase filtrando pelo dia (usando a coluna 'date' que já temos)
        ...
        // Filtra por local se selecionado (usa meeting_location_name, não location_id)
        if (locFilter && locFilter !== 'all') {
            query = query.eq('meeting_location_name', locFilter);
        }
        ...
        if (!sessions || sessions.length === 0) {
            container.innerHTML = '<p style="padding:30px;text-align:center;opacity:0.4;">Nenhuma presença encontrada para este dia.</p>';
            return;
        }

        // Monta lista de TODOS os participantes de todas as sessões do dia
        const allAttendees = [];

        sessions.forEach(sess => {
            const locationName = sess.meeting_location_name 
                || sess.location_name 
                || (sess.meeting_location_address ? sess.meeting_location_address.split(',')[0] : null)
                || 'Local não registrado';
            const hiddenKey = `attendance_hidden_${sess.id}`;
            const hiddenUids = JSON.parse(localStorage.getItem(hiddenKey) || '[]');

            // Adiciona o MOTORISTA (se não foi ocultado pelo gestor)
            if (sess.driver_id && !hiddenUids.includes(sess.driver_id)) {
                allAttendees.push({
                    uid: sess.driver_id,
                    name: sess.driver_name || 'Motorista',
                    role: 'motorista',
                    ts: sess.created_at,
                    locationName,
                    sessionId: sess.id
            ...
            // Adiciona os CARONAS (se não foram ocultados pelo gestor)
            const passengers = sess.passengers || [];
            passengers.forEach(p => {
                if (!p.canceled && !hiddenUids.includes(p.uid)) {
                    allAttendees.push({
                        uid: p.uid || '',
                        name: p.name || 'Carona',
                        role: p.boarded ? 'carona' : 'aguardando',
                        ts: p.boarding_ts || sess.created_at,
                        locationName,
                        sessionId: sess.id
                    });
                }
        ...
        if (allAttendees.length === 0) {
            container.innerHTML = '<p style="padding:30px;text-align:center;opacity:0.4;">Nenhuma presença encontrada.</p>';
            return;
        }

        // Ordena por horário
        allAttendees.sort((a, b) => new Date(a.ts) - new Date(b.ts));

        const roleConfig = {
            motorista: { color: '#3b82f6', label: 'MOTORISTA', icon: '🚗' },
            carona: { color: '#BF9A56', label: 'CARONA', icon: '👤' },
            individual: { color: '#10b981', label: 'INDIVIDUAL', icon: '🚶' },
            aguardando: { color: '#6b7280', label: 'AGUARDANDO', icon: '⏳' },
        };

        container.innerHTML = `
            <div style="padding:0 4px;">
                <div style="display:grid; grid-template-columns:1fr auto auto; gap:10px; padding:8px 12px; margin-bottom:8px; font-size:0.65rem; color:rgba(255,255,255,0.4); text-transform:uppercase; letter-spacing:1px; font-weight:700; border-bottom:1px solid rgba(255,255,255,0.05);">
                    <span>Nome</span><span>Horário</span><span>Função</span>
                </div>
                ${allAttendees.map(p => {
            const cfg = roleConfig[p.role] || roleConfig['carona'];
            const time = new Date(p.ts).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' });
            const safeUid = (p.uid || '').replace(/'/g, '');
            return `
                        <div style="display:grid; grid-template-columns:1fr auto auto auto; gap:10px; align-items:center; padding:10px 12px; margin-bottom:6px; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.05); border-radius:12px;">
                            <div>
                                <div style="font-weight:700; font-size:0.88rem;">${cfg.icon} ${p.name}</div>
                                <div style="font-size:0.65rem; color:rgba(255,255,255,0.4); margin-top:2px;">${p.locationName}</div>
                            </div>
                            <div style="font-size:0.78rem; font-weight:600; color:rgba(255,255,255,0.6); white-space:nowrap;">${time}</div>
                            <span style="font-size:0.6rem; font-weight:800; padding:3px 10px; border-radius:20px; background:${cfg.color}18; color:${cfg.color}; border:1px solid ${cfg.color}33; white-space:nowrap;">
                                ${cfg.label}
                            </span>
                            <button onclick="removeAttendeePresence('${p.sessionId}', '${safeUid}', '${p.role}')" title="Remover desta lista" style="background:rgba(239,68,68,0.1); border:1px solid rgba(239,68,68,0.25); color:#ef4444; border-radius:8px; padding:5px 8px; cursor:pointer; display:flex; align-items:center; gap:4px; font-size:0.68rem; white-space:nowrap; flex-shrink:0;">
        ...
        container.innerHTML = '<p style="padding:20px;text-align:center;color:#ef4444;">Erro ao carregar dados.</p>';
    }
}

// ── Remove UMA PESSOA da lista de presença sem apagar a sessão inteira ──
// Remove UMA PESSOA da lista de presença (sem tocar no banco)
// Usa localStorage para ocultar sem exigir coluna extra no Supabase
async function removeAttendeePresence(sessionId, uid, role) {
    const personType = role === 'motorista' ? 'motorista' : 'carona';

    const confirmed = await Swal.fire({
        title: 'Remover da Lista de Presença?',
        text: `O ${personType} será removido desta lista. A rota em Revisão de Reuniões não será afetada.`,
        icon: 'warning',
    ...
    // Salva no localStorage — sem alterar o banco de dados
    const key = `attendance_hidden_${sessionId}`;
    ...
    await Swal.fire({
        title: 'Removido!',
        text: 'Presença removida da lista. A rota está preservada em Revisão de Reuniões.',
        icon: 'success',
        timer: 1800,
...
function renderMeetingReviewCard(d) {
    const isActive = d.status !== 'finished' && d.status !== 'finalized' && d.status !== 'cancelled';
    ...
    if (passengersCount > 0) {
        const paxNames = passengers.map(p => {
            const status = (p.boarded && p.dropped_off) ? '(ida/volta)' : p.boarded ? '(ida)' : p.dropped_off ? '(volta)' : '';
            return `<span style="opacity:0.6">${p.name} <b style="color:var(--gold)">${status}</b></span>`;
        }).join(', ');
        passengersListHtml = `<div style="font-size:0.7rem; margin-top:8px; border-top:1px solid rgba(255,255,255,0.05); padding-top:8px;">
            <b style="opacity:0.4; text-transform:uppercase; font-size:0.6rem; letter-spacing:0.5px;">Caronas:</b> ${paxNames}
        </div>`;
    }

    // ── KM Real: calcula a partir dos pontos GPS (não depende de total_km) ──
    const allPoints = [...(d.gps_track || []), ...(d.return_track || [])];
    let kmReal = 0;
    if (allPoints.length > 1) {
        for (let i = 1; i < allPoints.length; i++) {
            const p1 = allPoints[i - 1], p2 = allPoints[i];
            const lat1 = Number(p1.lat), lon1 = Number(p1.lng ?? p1.lon);
            const lat2 = Number(p2.lat), lon2 = Number(p2.lng ?? p2.lon);
            if (isNaN(lat1) || isNaN(lon1) || isNaN(lat2) || isNaN(lon2)) continue;
            const dLat = (lat2 - lat1) * Math.PI / 180;
            const dLon = (lon2 - lon1) * Math.PI / 180;
            const a = Math.sin(dLat/2)**2 + Math.cos(lat1*Math.PI/180)*Math.cos(lat2*Math.PI/180)*Math.sin(dLon/2)**2;
            kmReal += 6371 * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
        }
    }
    if (kmReal === 0) kmReal = parseFloat(d.total_km || 0);

    // ── Status: tradução amigável com cores ──
    const statusMap = {
        'idle':       { label: 'Aguardando',  color: '#64748b' },
        'outbound':   { label: 'Indo',        color: '#3b82f6' },
        'at_meeting': { label: 'Na Reunião',  color: '#10b981' },
        'returning':  { label: 'Retornando',  color: '#f59e0b' },
        'inbound':    { label: 'Retornando',  color: '#f59e0b' },
        'return':     { label: 'Retornando',  color: '#f59e0b' },
        'finished':   { label: 'Finalizado',  color: '#8b5cf6' },
        'finalized':  { label: 'Finalizado',  color: '#8b5cf6' },
        'cancelled':  { label: 'Cancelado',   color: '#ef4444' },
    };
    const st = statusMap[d.status] || { label: d.status || '—', color: '#64748b' };

    // ── Local: tenta vários campos ──
    const locationName = d.meeting_location_name
        || d.location_name
        || (d.meeting_location_address ? d.meeting_location_address.split(',')[0] : null)
        || 'Local não registrado';
        ...
        <div style="display:flex; justify-content:space-between; align-items:flex-start;">
            <div style="flex:1;">
                ...
                <div style="font-size:0.75rem; opacity:0.7; display:flex; flex-wrap:wrap; gap:10px; align-items:center; margin-bottom:8px;">
                    ...
                    <span>${vIcon} ${d.vehicle_type || 'carro'} · ${passengersCount} carona(s)</span>
                </div>
                ...
                <div style="margin-top:16px; display:flex; gap:10px; flex-wrap:wrap;">
                    ...
                    <button class="btn" style="background:rgba(239,68,68,0.12); color:#ef4444; border:1px solid rgba(239,68,68,0.3); padding:6px 12px; border-radius:10px; display:flex; align-items:center; gap:5px; font-size:0.72rem;" onclick="deleteMeetingSession('${d.id}')">
                         <i data-lucide="trash-2" style="width:13px;"></i> Excluir
                    </button>
                </div>
            </div>
            
            <div style="text-align:right; flex-shrink:0; margin-left:12px;">
                <div style="font-weight:800; color:var(--gold); font-size:1.2rem; letter-spacing:-0.5px;">${kmReal.toFixed(2)} km</div>
                <div style="font-size:0.6rem; padding:3px 10px; margin-top:6px; border-radius:20px; font-weight:700; display:inline-block; letter-spacing:0.5px; background:${st.color}22; color:${st.color}; border:1px solid ${st.color}44;">${st.label}</div>
            </div>
        </div>
    </div>`;
}
...
async function deleteMeetingSession(sessionId) {
    ...
    try {
        ...
        // Recarrega o painel que estiver visível no momento
        const attendanceModal = document.getElementById('modal-meeting-attendance');
        const isAttendanceOpen = attendanceModal && !attendanceModal.classList.contains('hidden');

        if (isAttendanceOpen) {
            loadAttendanceList(); // Recarrega lista de presença
        } else {
            loadMeetingReview(); // Recarrega a auditoria de rotas
        }
    } catch (e) {
...
async function showDriverRouteDetailFromSession(sessionId) {
    ...
    try {
        const { data: d } = await window.supabase.from('meeting_sessions').select('*').eq('id', sessionId).maybeSingle();
        if (!d) return;

        console.log("[Auditoria] Detalhando Sessão:", d.id);
        ...
        const paxInfoEl = document.getElementById('real-passengers-info');

        const rate = (d.vehicle_type === 'moto') ? 0.40 : 0.90;

        // Calcula KM REAL a partir dos pontos de GPS (mais preciso do que o campo total_km que pode estar zerado)
        const track = [...(d.gps_track || []), ...(d.return_track || [])];
        const totalKmCalculated = _haversineKm(track);

        const totalKm = totalKmCalculated > 0 ? totalKmCalculated : (parseFloat(d.total_km || 0));
        ...
        const realKmInfoEl = document.getElementById('real-km-info');
        if (realKmInfoEl) realKmInfoEl.textContent = `KM REAL: ${totalKm.toFixed(2)} km`;

        // Lógica de Caronas (Ida/Volta)
        const passengers = d.passengers || [];
        const paxUids = passengers.map(p => p.uid);

        // Busca endereços de todos na tabela usuarios
        const { data: sellers } = await window.supabase.from('usuarios').select('uid, address, name').in('uid', [d.driver_id, ...paxUids]);
        const sellerMap = (sellers || []).reduce((acc, s) => ({ ...acc, [s.uid]: s }), {});

        // Caronas Info Display
        if (paxInfoEl) {
            if (passengers.length > 0) {
                paxInfoEl.innerHTML = '<strong>Caronas:</strong><br>' + passengers.map(p => {
                    const status = (p.boarded && p.dropped_off) ? '(Ida/Volta)' : p.boarded ? '(Ida)' : p.dropped_off ? '(Volta)' : '(N/A)';
                    return `${p.name} ${status}`;
                }).join('<br>');
            } else {
                paxInfoEl.innerHTML = '<strong>Viagem Solo</strong>';
            }
        }

        // 1. CONSTRUÇÃO DA ROTA PREVISTA (Dinamico)
        const driverHome = sellerMap[d.driver_id]?.address;
        const meetingLocAddr = d.meeting_location_address;

        // Stops Ida
        const stopsIda = [];
        if (driverHome) stopsIda.push({ label: 'Início (Casa)', address: driverHome });
        passengers.filter(p => p.boarded).forEach(p => {
            const addr = sellerMap[p.uid]?.address;
            if (addr) stopsIda.push({ label: `Embarque: ${p.name}`, address: addr });
        });
        if (meetingLocAddr) stopsIda.push({ label: 'Reunião', address: meetingLocAddr });

        // Stops Volta
        const stopsVolta = [];
        // Se já temos Reunião no fim da ida, começamos dela na volta
        passengers.filter(p => p.dropped_off).forEach(p => {
            const addr = sellerMap[p.uid]?.address;
            if (addr) stopsVolta.push({ label: `Desembarque: ${p.name}`, address: addr });
        });
        if (driverHome) stopsVolta.push({ label: 'Fim (Casa)', address: driverHome });

        const fullPredictedStops = [...stopsIda, ...stopsVolta];

        const stopsListEl = document.getElementById('pred-route-stops');
        const mapPredEl = document.getElementById('map-predicted');

        if (fullPredictedStops.length > 1) {
            if (stopsListEl) {
                stopsListEl.innerHTML = fullPredictedStops.map((s, i) => `
                    <div style="display:flex;gap:10px;margin-bottom:8px;align-items:flex-start;">
                        ...
                        <div>
                            <div style="font-size:0.75rem;font-weight:700;color:#3b82f6;">${s.label}</div>
                            <div style="font-size:0.65rem;opacity:0.6;line-height:1.2;">${s.address}</div>
                        </div>
                    </div>`).join('');
            }
            setTimeout(() => _renderPredictedRouteMap('map-predicted', fullPredictedStops, d.vehicle_type), 200);
        } else {
            let reason = "Informações de endereço insuficientes.";
            if (!driverHome) reason = "Motorista sem endereço residencial cadastrado.";
            else if (!meetingLocAddr) reason = "Local de reunião sem endereço definido.";

            if (stopsListEl) stopsListEl.innerHTML = `<p style="font-size:0.75rem;opacity:0.5;text-align:center;padding:10px;">${reason}</p>`;
            if (mapPredEl) {
                mapPredEl.innerHTML = `
                    <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; gap:12px; padding:20px; text-align:center; background:rgba(0,0,0,0.2);">
                        <i data-lucide="map-pin-off" style="width:32px; height:32px; color:var(--gold); opacity:0.3;"></i>
                        <div style="font-size:0.75rem; opacity:0.6;">${reason}</div>
                        <div style="font-size:0.65rem; opacity:0.4;">Verifique o cadastro em 'Vendedores Cadastrados' e insira o endereço de casa deste motorista.</div>
                    </div>
                `;
            }
        }

        // 2. RENDERIZA TRAJETO REAL + CHECKPOINTS
        // Reutiliza o 'track' já declarado acima

        // Extrai checkpoints dos tracks se a coluna dedicada não existir (ou estiver vazia)
        let checkpoints = d.checkpoints || [];
        if (!checkpoints || checkpoints.length === 0) {
            checkpoints = track.filter(p => p.type && (p.type !== 'breadcrumb' && p.type !== 'gps'));
        }

        setTimeout(() => {
            if (typeof _renderAuditedRouteMap === 'function') {
                _renderAuditedRouteMap('map-real-track', track, checkpoints);
            }
        }, 400);
        ...
        if (checkList) {
            checkList.innerHTML = checkpoints.length ? checkpoints.map(c => {
                const icon = (c.type === 'start' || c.type === 'return_start') ? 'play' :
                    (c.type === 'arrival' || c.type === 'meeting' || c.type === 'boarding' || c.type === 'dropoff') ? 'map-pin' : 'home';
                const color = (c.type === 'start' || c.type === 'return_start') ? '#3b82f6' :
                    (c.type === 'arrival' || c.type === 'boarding') ? '#10b981' :
                        (c.type === 'dropoff') ? '#ef4444' : '#f59e0b';
                return `
                <div style="display:flex;gap:10px;margin-bottom:8px;align-items:flex-start;">
                    <div style="width:20px;height:20px;border-radius:50%;background:${color}20;color:${color};display:flex;align-items:center;justify-content:center;font-size:0.7rem;font-weight:700;flex-shrink:0;border:1px solid ${color}40;"><i data-lucide="${icon}" style="width:12px;"></i></div>
                    <div>
                        <div style="font-size:0.75rem;font-weight:700;color:${color};">${c.label || 'EVENTO'}</div>
                        <div style="font-size:0.65rem;opacity:0.6;">${c.ts ? new Date(c.ts).toLocaleTimeString('pt-BR') : '--:--'} · Realizado</div>
                    </div>
                </div>`;
            }).join('') : '<p style="font-size:0.75rem;opacity:0.5;text-align:center;">Nenhum sinal de GPS auditado.</p>';
        }
...
// ── Ícones customizados para o mapa de auditoria ──
function _getAuditMarkerIcon(type) {
    const svgIcons = {
        // 🚩 Bandeira vermelha — Início da Rota
        start: `<svg xmlns="http://www.w3.org/2000/svg" width="40" height="48" viewBox="0 0 40 48"><circle cx="20" cy="20" r="18" fill="#ef4444" stroke="white" stroke-width="2.5"/><line x1="14" y1="10" x2="14" y2="30" stroke="white" stroke-width="2.5" stroke-linecap="round"/><polygon points="14,10 28,14 14,18" fill="white"/><circle cx="20" cy="20" r="18" fill="none" stroke="rgba(239,68,68,0.4)" stroke-width="6"/></svg>`,
        return_start: `<svg xmlns="http://www.w3.org/2000/svg" width="40" height="48" viewBox="0 0 40 48"><circle cx="20" cy="20" r="18" fill="#ef4444" stroke="white" stroke-width="2.5"/><line x1="14" y1="10" x2="14" y2="30" stroke="white" stroke-width="2.5" stroke-linecap="round"/><polygon points="14,10 28,14 14,18" fill="white"/></svg>`,
        // 🏢 Prédio — Local da Reunião
        arrival: `<svg xmlns="http://www.w3.org/2000/svg" width="40" height="48" viewBox="0 0 40 48"><circle cx="20" cy="20" r="18" fill="#3b82f6" stroke="white" stroke-width="2.5"/><rect x="12" y="13" width="16" height="15" rx="1" fill="white"/><rect x="15" y="9" width="10" height="6" rx="1" fill="rgba(255,255,255,0.8)"/><rect x="15" y="23" width="3" height="5" fill="#3b82f6" rx="0.5"/><rect x="22" y="23" width="3" height="5" fill="#3b82f6" rx="0.5"/><rect x="15" y="15" width="2.5" height="2.5" fill="#3b82f6" rx="0.3"/><rect x="19" y="15" width="2.5" height="2.5" fill="#3b82f6" rx="0.3"/><rect x="23" y="15" width="2.5" height="2.5" fill="#3b82f6" rx="0.3"/><rect x="15" y="19" width="2.5" height="2.5" fill="#3b82f6" rx="0.3"/><rect x="19" y="19" width="2.5" height="2.5" fill="#3b82f6" rx="0.3"/><rect x="23" y="19" width="2.5" height="2.5" fill="#3b82f6" rx="0.3"/></svg>`,
        meeting: `<svg xmlns="http://www.w3.org/2000/svg" width="40" height="48" viewBox="0 0 40 48"><circle cx="20" cy="20" r="18" fill="#3b82f6" stroke="white" stroke-width="2.5"/><rect x="12" y="13" width="16" height="15" rx="1" fill="white"/><rect x="15" y="9" width="10" height="6" rx="1" fill="rgba(255,255,255,0.8)"/><rect x="15" y="23" width="3" height="5" fill="#3b82f6" rx="0.5"/><rect x="22" y="23" width="3" height="5" fill="#3b82f6" rx="0.5"/><rect x="15" y="15" width="2.5" height="2.5" fill="#3b82f6" rx="0.3"/><rect x="19" y="15" width="2.5" height="2.5" fill="#3b82f6" rx="0.3"/><rect x="23" y="15" width="2.5" height="2.5" fill="#3b82f6" rx="0.3"/><rect x="15" y="19" width="2.5" height="2.5" fill="#3b82f6" rx="0.3"/><rect x="19" y="19" width="2.5" height="2.5" fill="#3b82f6" rx="0.3"/><rect x="23" y="19" width="2.5" height="2.5" fill="#3b82f6" rx="0.3"/></svg>`,
        // 👤 Boneco — Embarque/Desembarque de Carona
        boarding: `<svg xmlns="http://www.w3.org/2000/svg" width="40" height="48" viewBox="0 0 40 48"><circle cx="20" cy="20" r="18" fill="#f59e0b" stroke="white" stroke-width="2.5"/><circle cx="20" cy="14" r="4.5" fill="white"/><path d="M11 30 Q11 21 20 21 Q29 21 29 30" fill="white"/></svg>`,
        dropoff: `<svg xmlns="http://www.w3.org/2000/svg" width="40" height="48" viewBox="0 0 40 48"><circle cx="20" cy="20" r="18" fill="#e67e22" stroke="white" stroke-width="2.5"/><circle cx="20" cy="14" r="4.5" fill="white"/><path d="M11 30 Q11 21 20 21 Q29 21 29 30" fill="white"/></svg>`,
        // 🏠 Casa vermelha — Fim da Jornada
        end: `<svg xmlns="http://www.w3.org/2000/svg" width="40" height="48" viewBox="0 0 40 48"><circle cx="20" cy="20" r="18" fill="#dc2626" stroke="white" stroke-width="2.5"/><path d="M20 9 L30 19 L27 19 L27 30 L13 30 L13 19 L10 19 Z" fill="white"/><rect x="17" y="22" width="6" height="8" fill="#dc2626" rx="1"/></svg>`,
    };
    return svgIcons[type] || svgIcons['boarding'];
}

function _makeMarkerIcon(svgStr, size) {
    const sz = size || 40;
    return {
...
function _renderAuditedRouteMap(containerId, track, checkpoints) {
    ...
    if (track && track.length > 0) {
        const path = track
            .map(p => {
                ...
                const lng = p.lng !== undefined ? Number(p.lng) : Number(p.lon);
    ...
    (checkpoints || []).forEach(cp => {
        ...
        const lng = cp.lng !== undefined ? Number(cp.lng) : Number(cp.lon);
        if (isNaN(lat) || isNaN(lng)) return;

        const svgStr = _getAuditMarkerIcon(cp.type);
        new google.maps.Marker({
            position: { lat, lng },
            map,
            title: cp.label || 'Evento',
            icon: _makeMarkerIcon(svgStr, 40)
        });
    });
}

function _renderPredictedRouteMap(containerId, stops, vehicleType) {
    const el = document.getElementById(containerId);
    ...
    const directionsRenderer = new google.maps.DirectionsRenderer({
        map,
        suppressMarkers: true, // Vamos adicionar ícones customizados
        polylineOptions: { strokeColor: '#3b82f6', strokeWeight: 4, strokeOpacity: 0.85 }
    });

    // Remove duplicatas consecutivas de endereços (ex: Reunião no fim da ida e início da volta)
    const cleanedStops = [];
    stops.forEach((s, i) => {
        if (i === 0 || s.address !== stops[i - 1].address) cleanedStops.push(s);
    });

    const origin = cleanedStops[0].address;
    const destination = cleanedStops[cleanedStops.length - 1].address;
    const waypoints = cleanedStops.length > 2 ? cleanedStops.slice(1, -1).map(s => ({
        location: s.address,
        stopover: true
    })) : [];
    ...
    directionsService.route({
        ...
        if (status === 'OK') {
            ...
            // Adiciona ícones customizados no lugar dos marcadores A,B,C...
            const legs = result.routes[0].legs;
            cleanedStops.forEach((stop, i) => {
                let iconType = 'boarding';
                const label = stop.label || '';
                if (i === 0) iconType = 'start';
                else if (i === cleanedStops.length - 1) iconType = 'end';
                else if (label.toLowerCase().includes('reuni')) iconType = 'meeting';
                else if (label.toLowerCase().includes('embarque') || label.toLowerCase().includes('desembarque')) iconType = 'boarding';

                const pos = i === 0 ? legs[0].start_location
                    : i < legs.length ? legs[i - 1].end_location
            ...
            const predKmInfoEl = document.getElementById('pred-km-info');

            if (predKmEl) predKmEl.textContent = `${totalKmPrev.toFixed(2)} km`;
            if (predPayEl) predPayEl.textContent = `R$ ${custoPrev.toFixed(2)}`;
            if (predKmInfoEl) predKmInfoEl.textContent = `KM PREVISTO: ${totalKmPrev.toFixed(2)} km`;
        } else {
            el.innerHTML = `<div style="padding:20px; text-align:center; font-size:0.7rem; opacity:0.5;">Erro lógica de rota: ${status}</div>`;
        }
    });
}
